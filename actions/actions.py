...
API_KEY = "7b60fbeb94404eee3cd2d2eb5327848f"  # config['API_KEY']
# config['RAPID_API_KEY']
RAPID_API_KEY = "b2b8df2c4emsh55932ebbbefe351p11f903jsn956c777b38ca"
# config['AMADEUS_CLIENT_ID']
AMADEUS_CLIENT_ID = "PLOIijL3JBJf7pKgABAfLZOfdjBLP0iH"
AMADEUS_CLIENT_SECRET = "GSiJxTIDRXWJmpvR"  # config['AMADEUS_CLIENT_SECRET']

IATA_MAPPER = {}
filename = "./EasyPNRAirports.csv"
with open(filename, 'r', encoding='utf8', errors="ignore") as data:
...
class ActionCheckWeather(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        response = """It is currently <b>{}</b> in {} at the moment. The temperature is <b>{}</b> degrees, the humidity is <b>{}%</b> and the wind speed is <b>{}</b> mph.""".format(
...
class ActionCheckHotels(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_message(
            f"Showing Hotel Suggestions in {hotel_details['term']}....")
...
class ActionCheckStations(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        dispatcher.utter_message(
            f"Listing Suggestions for transportation stations in {hotel_details['term']}....")
        for group in hotel_details['suggestions']:
...
class ActionBestHotel(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        loc = tracker.get_slot('location')
        querystring = {"cityName": loc, "countryName": "United Kingdom"}
        ...
        dispatcher.utter_message(
            f"<a href = {hotel_details['link']} target='_blank'>Click Here to Book</a>")
...
class ValidateFlightForm(FormValidationAction):
        ...
        with open(filename, 'r', encoding='utf8', errors="ignore") as data:
            for line in csv.DictReader(data):
                ...
                if slot_value.lower() in line['Location'].lower():
                    dispatcher.utter_message(
                        f"I found {line['Airport name']} for {slot_value}")
                    return {"origin_location": line['iataCode']}
        dispatcher.utter_message(
            f"I couldn't find an airport for the location <b>{slot_value}</b>, Please Enter proper location.")
    ...
    def validate_departure_date(
        ...
        if re.match(regex, slot_value) == None:
            dispatcher.utter_message(
                "I'm assuming you mis-spelled the departure date. Here's the format of date: yyyy-mm-dd")
            return {"departure_date": None}
        elif datetime.strptime(str(date.today()), '%Y-%m-%d') > datetime.strptime(slot_value, '%Y-%m-%d'):
            dispatcher.utter_message(
                "Hey, I'm sorry to inform that time travel is yet to be discovered 🙁! Untill then, please Enter a date either in future or today's date.")
            return {"departure_date": None}
        else:
    ...
    def validate_noof_adults(
        ...
        if re.match(regex, slot_value) == None:
            dispatcher.utter_message(
                "You can book tickets for 1-9 adults. Please Enter numbers in only range of 1 to 9.")
...
class ActionSubmit(Action):
    ...
    def run(
        ...
        url = "https://test.api.amadeus.com/v2/shopping/flight-offers?originLocationCode={}&destinationLocationCode={}&departureDate={}&adults={}&currencyCode=INR&nonStop=false&max=250".format(
            origin_location, destination_location, departure_date, noof_adults)
        ...
        # Now, respond to user
        dispatcher.utter_message(
            f"<b>{data['meta']['count']}</b> flights found ....")
